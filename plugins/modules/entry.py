...
def main():
    ...
    if icon_id is not None:
        if icon_id > 68:
            module.fail_json(msg='Icon_id out of range. Choose a value between 0 and 68')
    else:
        icon_id = 58
    ...
    if group is None:
        module.fail_json(msg=f"Group does not exist: '{group_path}'")
    ...
    if action.lower() == "create":
        ...
        if not module.check_mode:
            try:
                ...
                module.fail_json(msg=f'Could not add the entry or save the database: {str(e)}')
        ...
        entry = kp.find_entries(title=title, group=group, recursive=False, first=True)
        if entry is None:
            module.fail_json(msg=f"No entry found in database with title '{title}' in group '{group_path or 'root'}'")
        else:
            entry.save_history()
            ...
            try:
                ...
                module.fail_json(msg=f'Could not save the database after modifying entry: {str(e)}')
        ...
        if entry is None:
            ...
            try:
                ...
                module.fail_json(msg=f'Could not delete the entry or save the database: {str(e)}')
...
def set_result(entry, changed):
    ...
    result['group_path']        = ""
    for directory in entry.group.path:
        result['group_path'] += directory + "/"
