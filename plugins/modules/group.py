...
options:
    ...
    create_path:
        description:
            - Indicator that specifies whether the path should be created if it doesn't exist.
        required: false
        choices: ['true', 'false']
        type: boolean
author:
...
def main():
    ...
    if (action.lower() == "create" and create_path is None) :
        module.fail_json(msg="If Action 'Create' is given you need to set 'create_path' to specify whether the given path should be created if it doesn't already exist")
    ...
    if action.lower() == "create":
        ...
        if not module.check_mode:
            try:
                ...
                if not group and create_path is True:
                    ...
                    module.fail_json(msg=f"Path does not exist: '{path}'. Set 'create_path' to True to create it")
                else:
                    pass
                if not icon_id:
                    icon_id = 48
                kp.add_group(kp.find_groups(path=directory_list,first=True), group_name=name, icon=str(icon_id), notes=notes or 'Generated by ansible.')
                kp.save()
            except Exception as e:
                module.fail_json(msg=f'Could not add the group or save the database: {str(e)}')
        ...
        group = kp.find_groups(path=directory_list + [name], first=True)
        if group is None:
            module.fail_json(msg=f"No group found in database with name '{name}' in path '{path or 'root'}'")
        else:
            if notes:
                group.notes = notes
            ...
            try:
                ...
                module.fail_json(msg=f'Could not save the database after modifying group: {str(e)}')
        ...
        if group is None:
            ...
            try:
                ...
                module.fail_json(msg=f'Could not delete the group or save the database: {str(e)}')
            module.exit_json(changed=True)

    else:
        module.fail_json(msg=f"Invalid action '{action}'. Must be 'create', 'modify', or 'delete'")
...
def set_result(group, changed):
    ...
    result['full_path']     = ""
    for directory in group.path:
        result['full_path'] += directory + "/"
